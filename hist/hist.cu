...
__global__
static void histWithSharedMemory(int nbins, int *dbins, int nvals, const int *dvals)
{
...
static void hist(int nbins, int *dbins, int nvals, const int *dvals)
{
    ...
    if (index < nvals) {
      int value = dvals[index];
      int binValue = value % nbins;
      int* binAddress = dbins + binValue;
